...
@Component({
  selector: 'app-inventory-product-abc-filters',
  templateUrl: './inventory-product-abc-filters.component.html',
  styleUrls: ['./inventory-product-abc-filters.component.scss'],
  providers: [DatePipe]
})
export class InventoryProductAbcFiltersComponent implements OnInit { 
  @Input() expanded = false;
  @Input() filters:  InventoryProductAbcFilters;
  @Input() loading  = false;
  @Input() cboactive: number;
  @Input() dataUnavailable = true;
  @Output() search = new EventEmitter<InventoryProductAbcFilters>();
  ...
  completeFilterModel() {
    ...
    this.filters.idSupplier = this.checkIfStringEmpty(this.supplierBrandFilters.idsupplier);
    this.filters.idBrand = this.checkIfStringEmpty(this.supplierBrandFilters.idbrand);
    this.filters.idBranchOffice = this._authService.currentOffice;
  }
