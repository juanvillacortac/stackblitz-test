...
export class WastageDialogComponent implements OnInit {
  ...
  constructor(private _wastageservice: WastageService, private messageService: MessageService, private confirmationService: ConfirmationService) { }
  ...
  saveWastage(): void {
    this.submitted=true;
    if (this._wastage.name.trim()) {
      this._wastage.id == 0 ? -1 : this._wastage.id;
      this._wastage.name = this._wastage.name.trim();
      this._wastage.name = this._wastage.name.toLocaleUpperCase();
       //this._wastage.name = this._wastage.name.charAt(0).toLocaleUpperCase() + this._wastage.name.substr(1).toLowerCase();
       if(this._wastage.active || !this.activeReg){
              this.submit();
       }else if(this.activeReg && this._wastage.active==false){
          this.confirmationService.confirm({
           header: 'Confirmación',
           icon: 'pi pi-exclamation-triangle',
            message: 'Si inactiva el tipo de merma las configuraciones asociadas\ a esta se dejarán de visualizar, ¿desea proceder con la acción?',
            accept: () => {
               this.submit();
              },
              });
