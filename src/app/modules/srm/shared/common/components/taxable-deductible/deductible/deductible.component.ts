...
import { ApplyCost } from '../../../../Utils/apply-cost';
import { DistributionCost } from '../../../../Utils/distribution-cost';
import { Typevalue } from '../../../../view-models/common/typevalue';
...
export class DeductibleComponent implements OnInit {
  ...
  deductibles: Deductible = new Deductible();
  typeDiscountList: SelectItem[] = [];
  type: Typevalue[];
  typelist: SelectItem[] = [];
  deductiblesListTemp: Deductible[] = [];
  ...
  @Input("_productsSelecteds") _productsSelecteds: PurchaseOrderProduct[];
  isCost: boolean = true;
  cont: number = 0;
  ...
  onShowDeductible() {
    ...
    this.GetTaxableType();
    debugger
    if (this.ProductOrder == undefined && (this._productsSelecteds == undefined || this._productsSelecteds.length==0)) {
      debugger
  ...
  deductibleTempAdd() {
    ...
    if (this.ProductOrder != undefined) {
      ...
      this.deductibles.idPurchaseOrder = this.PurchaseOrder.purchase.idOrderPurchase;
    }
  ...
  clear() {
    this.deductibles = new Deductible();
