...
@Component({
  ...
  styleUrls: ['./taxable.component.scss'],
...
export class TaxableComponent implements OnInit {
  ...
  TaxList: SelectItem[] = [];
  rateList: SelectItem[] = [];
  taxables: Taxable = new Taxable();
  txablesListTemp: Taxable[] = [];
  type: Typevalue[];
  typelist: SelectItem[] = [];
  ...
  taxableTempAdd() {
    ...
    if (this.ProductOrder != undefined) {
      ...
      this.taxables.idPurchaseOrder = this.PurchaseOrder.purchase.idOrderPurchase;
    }
