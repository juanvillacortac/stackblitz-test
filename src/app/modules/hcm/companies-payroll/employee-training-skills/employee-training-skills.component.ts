...
@Component({
  selector: 'app-employee-training-skills',
  templateUrl: './employee-training-skills.component.html',
  styleUrls: ['./employee-training-skills.component.scss']
})
export class EmployeeTrainingSkillsComponent implements OnInit {
  ...
  onLoadEmployeeskills(){
    ...
    this.employeeTrainingService.getEmployeeTraining(this.employeeTrainingFilter).subscribe((data: EmployeeTraining) => {
      ...
      this.skills = this.trainingEmployee.skills?.split(",");
    });
  }
  ...
  submit(){
    if(this.skills.length == 0){
      ...
      this.record.skills = this.skills.toString();
      this.record.idtype = 1;
